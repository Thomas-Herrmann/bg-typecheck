import Constraint
import ConstraintInclusion (constraintsInclude, findConical)
...
main = do
  ...
  print c3
  print cnew
  print (constraintsInclude (Set.fromList [c1, c2, c3]) cnew)
...
c1 = monIndex i 1 .+. nIndex (-3) :<=: zeroIndex -- 1i - 3 <= 0

c2 = monIndex j 1 .+. monIndex k 1 .+. nIndex (-2) :<=: zeroIndex -- 1j + 1k - 2 <= 0

c3 = monIndex k (-1) :<=: zeroIndex -- -1k <= 0
